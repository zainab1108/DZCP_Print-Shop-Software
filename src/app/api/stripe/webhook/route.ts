    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      await tx.payment.create({
        data: {
          ...
          notes: outcome.amountMismatch
            ? `Paid online. Amount differs from the balance when checkout started (expected ${outcome.expectedCents} cents) — the invoice changed mid-checkout; review for a possible refund.`
            : "Paid online by card.",
        },
      });
