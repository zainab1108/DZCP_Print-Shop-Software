...
import { formatDate, formatMoney, formatUnitPrice, quoteNumber } from "./format";
import {
  loadLogo,
  MUTED,
  pdfStyles as styles,
...
/** Render a customer-facing quote PDF. Internal notes are never included. */
export async function renderQuotePdf(data: QuotePdfData): Promise<Buffer> {
  ...
  return renderToBuffer(
    <Document
      title={`${quoteNumber(data.number)} — ${SHOP_NAME}`}
      author={SHOP_NAME}
    >
      <Page size="LETTER" style={styles.page}>
        <View style={styles.header}>
          ...
          <View>
            ...
            <Text style={styles.docNumber}>{quoteNumber(data.number)}</Text>
          </View>
        </View>
        ...
        <Text
          ...
          render={({ pageNumber, totalPages }) =>
            `${SHOP_NAME} · ${quoteNumber(data.number)} · Page ${pageNumber} of ${totalPages}`
          }
          fixed
        />
      </Page>
