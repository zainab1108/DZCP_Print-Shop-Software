...
async function main() {
  ...
  const dbTotal = imported.reduce(
    (s, i) => s.plus(i.total),
    new Prisma.Decimal(0),
  );
  const dbPaid = imported.reduce(
    (s, i) => s.plus(i.amountPaid),
    new Prisma.Decimal(0),
  );

  console.log(
    `Imported invoices: ${imported.length} (CSV had ${invoiceRows.length} + 3 orphan-payment invoices)`,
  );
  console.log(
    `CSV total:  ${csvTotal.toFixed(2)}   DB total:  ${dbTotal.toFixed(2)}`,
  );
  console.log(
    `CSV paid:   ${csvPaid.toFixed(2)}   DB paid:   ${dbPaid.toFixed(2)}  (DB includes 3 orphan invoices' amounts, so DB paid > CSV paid by their sum)`,
  );
  ...
  for (const inv of imported) {
    ...
    if (!lineSum.equals(inv.subtotal)) {
      lineItemMismatch++;
      console.warn(
        `  MISMATCH inv #${inv.number}: subtotal=${inv.subtotal} but lineItems sum=${lineSum}`,
      );
    }
  }
  ...
  for (const inv of imported) {
    if (!inv.subtotal.plus(inv.taxAmount).equals(inv.total)) {
      mathMismatch++;
      console.warn(
        `  MATH MISMATCH inv #${inv.number}: ${inv.subtotal} + ${inv.taxAmount} != ${inv.total}`,
      );
    }
  }
  ...
  for (const inv of imported) {
    const paySum = inv.payments.reduce(
      (s, p) => s.plus(p.amount),
      new Prisma.Decimal(0),
    );
    if (!paySum.equals(inv.amountPaid)) {
      paymentMismatch++;
      console.warn(
        `  PAYMENT MISMATCH inv #${inv.number} (${inv.title}): amountPaid=${inv.amountPaid} but payments sum=${paySum}`,
      );
    }
  }
